...
//    val totalTransport = MutableLiveData<Number>(0)
    ...
    fun calculateTransportByCity(city: String, pos: Int) {
        ...
        if (hasSameTransport(city, mode)) {
            ...
            transport.TransportationModeName = ""
    ...
    private fun compareCalculateTransportExpense(
        ...
        val transport = getTransportExpense(pos)
        val idMode =
            if (result is Result.Success) modeTransports.first { it.Text == mode }.ValueInt else transport.TransportationMode
        transport.TransportationType =
            if (result is Result.Success) type else transport.TransportationType
        transport.TransportationMode = idMode
        if (result is Result.Success) {
            val modeName =
                if (mode.isNotEmpty()) mode else modeTransports.first { it.ValueInt == idMode }.Text
            ...
            transport.City = city
            transport.TransportationModeName = modeName
    ...
    private fun hasSameTransport(city: String, mode: String): Boolean {
        return transportExpenses.any { it.City == city && it.TransportationModeName == mode }
    }
    ...
    fun indexFirstEmpty(): Int {
        val i = transportExpenses.indexOfFirst {
            it.City.isEmpty() || (it.TransportationType == NON_FLIGHT && it.TransportationModeName.isEmpty())
        }
        indexEmpty.set(i)
        return i
    }
    ...
    fun addingTransportExpense(list: List<TransportExpenses>) {
        ...
        list.forEach {
            ...
            data.TransportationModeName = getModeName(it.TransportationMode)
            data.TripType = it.TripType
            transportExpenses.add(data)
        }
    }

    private fun getModeName(modeId: Int): String {
        return modeTransports.first { it.ValueInt == modeId }.Text
    }
