...
class DetailParticipantActivity : BaseActivity()
    ...
    override fun OnMain() {
        ...
        id  = bundle.getString(Constants.KEY_INTENT_TRIPID)
        str = bundle.getString(Constants.Summary)
        status = bundle.getString(Constants.STATUS_MEMBER)
        jobTitle = bundle.getString(Constants.JOB_TITLE)
        nameParticipant = bundle.getString(Constants.NAME_PARTICIPANT)
        costCenter          = bundle.getString(Constants.COSTCENTER)
        budget          = bundle.getString(Constants.BUDGET)
        idParticipant = bundle.getString(Constants.ID_PARTICIPANT)
        employId      = bundle.getString(Constants.EMPLOY_ID)
