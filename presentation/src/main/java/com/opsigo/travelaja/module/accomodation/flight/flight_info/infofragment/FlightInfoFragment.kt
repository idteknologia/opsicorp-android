...
class FlightInfoFragment : BaseFragment(){
    ...
    override fun getLayout(): Int { return R.layout.flight_info_fragment }


    val data = ArrayList<ModelListDepartureAndArrivalAdapter>()
    ...
    private fun addData() {
        ...
        if(Globals.DATA_FLIGHT.isNotEmpty()) {
            ...
            val listData = ArrayList<ModelListDepartureAndArrivalAdapter>()

            if (data.isConnecting){
                data.transiteFlight.forEachIndexed { index, transiteFlight ->
                    val mData = ModelListDepartureAndArrivalAdapter()
